...
p_val <- nrow(mc_df[mc_df$Middle >= obs_rez_counts$Middle,]) / nrow(mc_df)
p_val
```
...
rez_acc_sf <- accidents_sf |> filter(reservation==TRUE)
rez_nb <- spdep::dnearneigh(rez_acc_sf, 0, 50000)
rez_listw <- spdep::nb2listw(rez_nb, zero.policy = TRUE)

rez_moran <- spdep::moran.test(rez_acc_sf$FATALS, listw = rez_listw)
rez_moran

off_acc_sf <- accidents_sf |> filter(reservation==FALSE)
off_nb <- spdep::dnearneigh(off_acc_sf, 0, 500000)
off_listw <- spdep::nb2listw(off_nb, zero.policy = TRUE)

off_moran <- spdep::moran.test(off_acc_sf$FATALS, listw = off_listw)
off_moran
```
